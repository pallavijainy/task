...
const ManagerDashboard = () => {
  ...
  const { data: pendingOvertimeData, isLoading: loadingOvertime } = useGetPendingOvertimeQuery();
    ...
    <DashboardLayout title="Team Management" subtitle={`${getGreeting()}, ${user?.name} 👋`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
        ...
        <StatCard
          ...
          value={pendingOvertimeData?.count || 0}
          icon={Clock}
          color="info"
        />
      </div>
      ...
      <Card>
        ...
        ) : !pendingOvertimeData?.overtime || pendingOvertimeData.overtime.length === 0 ? (
          <EmptyState
            icon={Clock}
            title="No pending overtime requests"
            description="Overtime requests from your team will appear here"
          />
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-slate-200">
                {pendingOvertimeData.overtime.map((ot) => (
